...
# Matrix creation
print("Exploring matrix creation")
a = np.zeros((1, 5))
print("np.zeros((1, 5)):")
print(a)
...
print("np.arange(6).reshape(-1, 2): The negative dimension means infer dimension size")
print(a)
print(f"shape = {a.shape}")
print()

# Matrix indexing
print("Exploring matrix indexing using a = np.arange(6).reshape(-1, 2)")
a = np.arange(6).reshape(-1, 2)
print(f"a[2] = {a[2]}, a[2].shape = {a[2].shape}, type(a[2]) = {type(a[2])}")
print(f"a[2][0] = {a[2][0]}, a[2][0].shape = {a[2][0].shape}, type(a[2][0]) = {type(a[2][0])}")
print(f"a[2, 0] = {a[2, 0]}, a[2, 0].shape = {a[2, 0].shape}, type(a[2, 0]) = {type(a[2, 0])}")
print()

# Matrix slicing
print("Exploring matrix slicing using a = np.arange(20).reshape(-1, 10)")
a = np.arange(20).reshape(-1, 10)
print(f"a[:, 1] = {a[:, 1]}")
print(f"a[:][1] = {a[:][1]} a[:] returns a full 2D array, so the second index operation [1] is selecting the second item of the full 2D array")
print(f"a[1, :] = {a[1, :]}")
print(f"a[1][:] = {a[1][:]}")
